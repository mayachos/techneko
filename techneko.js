...
function fadeAnime() {
    ...
    $('.fadeUpTrigger').each(function () { //fadeInUpTriggerというクラス名が
        ...
        var scroll = $(window).scrollTop(); var windowHeight = $(window).height(); if (scroll >= elemPos - windowHeight) {
    ...
    $('.fadeRightTrigger').each(function () { //fadeInUpTriggerというクラス名が
        var elemPos = $(this).offset().top - 5; //要素より、50px上の
        var scroll = $(window).scrollTop(); var windowHeight = $(window).height(); if (scroll >= elemPos - windowHeight) {
            $(this).addClass('fadeRight');　　 // 画面内に入ったらfadeInというクラス名を追記
        } else {
            $(this).removeClass('fadeRight');　　 // 画面外に出たらfadeInというクラス名を外す
        }
    });
